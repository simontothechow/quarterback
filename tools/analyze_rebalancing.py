...
if not b1_zero.empty:
    for _, row in b1_zero.iterrows():
        ...
        if pd.notna(shares) and shares > 0:
            print(
                f"  {row['TICKER']:12} - Need to BUY {shares:,.0f} shares (Index Weight: {row['INDEX_WEIGHT']*100:.4f}%)"
            )
else:
    print("  None")
...
print("\n" + "-" * 80)
print(
    f"Positions needing rebalancing (>{REBALANCE_THRESHOLD_SHARES} share diff, sorted by absolute share difference):"
)
print("-" * 80)
...
for _, row in b1_rebal_sorted.head(25).iterrows():
    ...
    print(
        f"  {row['TICKER']:12} - {action:4} {abs(shares):>8,.0f} shares | Current: {current_qty:>6,} | Target: {target_qty:>6,} | Price: ${price:>8.2f} | Value: ${value:>12,.2f}"
    )
...
if not b2_zero.empty:
    for _, row in b2_zero.iterrows():
        ...
        if pd.notna(shares) and shares > 0:
            print(
                f"  {row['TICKER']:12} - Need to SHORT {shares:,.0f} shares (Index Weight: {row['INDEX_WEIGHT']*100:.4f}%)"
            )
else:
    print("  None")
...
for _, row in b2_rebal_sorted.head(25).iterrows():
    ...
    value = abs(row["VALUE_DIFF"])
    print(
        f"  {row['TICKER']:12} - {action:10} {abs(shares):>8,.0f} shares | Current: {current_qty:>6,} | Target: {target_qty:>6,} | Price: ${price:>8.2f} | Value: ${value:>12,.2f}"
    )
