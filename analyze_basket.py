...
# Get Basket 1 data
basket1_equities = positions[(positions['BASKET_ID'] == 'Basket1') & (positions['POSITION_TYPE'] == 'EQUITY')]
basket1_futures = positions[(positions['BASKET_ID'] == 'Basket1') & (positions['POSITION_TYPE'] == 'FUTURE')]

# Key values
basket_notional = 100_000_000  # From futures position
...
for r in results[:20]:
    print(f"{r['ticker']:12} | Current: {int(r['qty']):>8,} | Target: {int(r['target']):>8,} | {r['action']:4} {abs(int(r['diff'])):>6,} shares")
...
for _, pos in basket1_equities.iterrows():
    ...
    ticker_clean = ticker.replace(' UN', '').replace(' UQ', '').replace(' US', '').strip()
    matches = market_data[market_data['BLOOMBERG_TICKER'].str.contains(ticker_clean, case=False, na=False)]
...
if bad_matches:
    ...
    for b in bad_matches[:10]:
        print(f"  {b['ticker']} (clean: '{b['clean']}') -> {b['matches']} matches: {b['matched_to']}")
