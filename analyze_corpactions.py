"""
Analyze corporate actions and calculate trade recommendations
"""
import pandas as pd

# Load all data
corp_actions = pd.read_csv('data/corpactions.csv')
...
for _, event in weight_changes.iterrows():
    ...
    ticker_positions = positions[
        (positions['UNDERLYING'] == ticker) & 
        (positions['POSITION_TYPE'] == 'EQUITY')
    ]
    ...
    for _, pos in ticker_positions.iterrows():
        ...
        print(f"{basket_id:<10} {int(current_qty):>10,} {int(new_target_shares):>12,} {action:>18} ${trade_value:>13,.0f}")
